...
<p>
	<?php
                   $dashbrd_current_user = wp_get_current_user();
				   $dashbrd_currentUserId = $current_user->data->ID;
				   
				   $dashbrd_user_info = get_userdata($dashbrd_currentUserId);
				   $dashbrd_LoggedInUserFirstName = $dashbrd_user_info->first_name; 
				   $dashbrd_LoggedInUserLoginName = $dashbrd_current_user->data->user_login;
				   
				   $dashbrd_NameForLogoutLink = "";
				   
				   if(isset($dashbrd_LoggedInUserFirstName)){
				    $dashbrd_NameForLogoutLink = $dashbrd_LoggedInUserFirstName;
				   }else{
				    $dashbrd_NameForLogoutLink = $dashbrd_LoggedInUserLoginName;
				   }
		...
		echo sprintf( esc_attr__( 'Hello %s%s%s (not %2$s? %sSign out%s)', 'woocommerce' ), '<strong>', esc_html($dashbrd_NameForLogoutLink), '</strong>', '<a href="' . esc_url( wc_get_endpoint_url( 'customer-logout', '', wc_get_page_permalink( 'myaccount' ) ) ) . '">', '</a>' );
	?>
</p>
