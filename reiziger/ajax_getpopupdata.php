<?php
	...
	$storeid = (isset($_GET['storeid'])) ? $_GET['storeid'] : $_POST['storeid'];
	$oldurl = (isset($_GET['oldurl'])) ? $_GET['oldurl'] : $_POST['oldurl'];
	...
	$sql = "SELECT posts.ID,
			...
			WHERE posts.post_type = 'wpsl_stores' AND posts.post_status = 'publish' AND (posts.ID = '".$storeid."')";
	
	$dealer = $wpdb->get_results($sql);
	$zip = $dealer[0]->zip;
	
	$find = "SELECT posts.ID,
					posts.post_title,
